...
const getAmenityIcon = (text: string) => {
  ...
  // WiFi & Tech
  if (t.includes("واي فاي") || t.includes("wifi") || t.includes("إنترنت") || t.includes("انترنت")) return <Wifi size={32} />;
  if (t.includes("تلفزيون") || t.includes("tv") || t.includes("شاشة")) return <Tv size={32} />;
  
  // Food & Drink
  if (t.includes("إفطار") || t.includes("فطار") || t.includes("breakfast") || t.includes("فطور")) return <Coffee size={32} />;
  if (t.includes("شاي") || t.includes("قهوة") || t.includes("tea") || t.includes("coffee") || t.includes("كيتل") || t.includes("آلة") || t.includes("الة")) return <Coffee size={32} />;
  ...
  // Transport
  if (t.includes("مطار") || t.includes("airport") || t.includes("طيران")) return <Plane size={32} />;
  if (t.includes("نقل") || t.includes("transfer") || t.includes("توصيل") || t.includes("ليموزين")) return <Car size={32} />;
  ...
  if (t.includes("خدمة غرف") || t.includes("خدمة الغرف") || t.includes("room service")) return <ConciergeBell size={32} />;
  if (t.includes("استقبال") || t.includes("reception") || t.includes("24 ساعة") || t.includes("مدار")) return <Clock3 size={32} />;
  if (t.includes("أمتعة") || t.includes("حقائب") || t.includes("luggage") || t.includes("baggage")) return <Luggage size={32} />;
  if (t.includes("غسيل") || t.includes("laundry") || t.includes("تنظيف")) return <WashingMachine size={32} />;
  if (t.includes("تكييف") || t.includes("ac") || t.includes("هواء") || t.includes("مكيف")) return <Wind size={32} />;
  ...
  if (t.includes("لا يدخن") || t.includes("non-smoking") || t.includes("مدخنين") || t.includes("تدخين")) return <CigaretteOff size={32} />;
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-bold text-gray-500">
                {(() => {
                  const raw = hotel.rating || 0;
                  const out5 = raw > 5 ? raw / 2 : raw;
                  const full = Math.floor(out5);
                  const half = out5 - full >= 0.5;
                  const empty = 5 - full - (half ? 1 : 0);
                  return (
                    <div className="flex items-center gap-0.5">
                      {Array.from({ length: full }).map((_, i) => (
                        <Star key={`f${i}`} size={16} className="fill-current text-amber-400" />
                      ))}
                      {half && (
                        <div className="relative w-4 h-4">
                          <Star size={16} className="text-gray-200 fill-current absolute inset-0" />
                      ...
                      {Array.from({ length: empty }).map((_, i) => (
                        <Star key={`e${i}`} size={16} className="fill-current text-gray-200" />
                      ))}
                      <span className="mr-1 text-xs font-black text-gray-400">{out5.toFixed(1)}</span>
                    </div>
                  );
                })()}
              </div>
